...
\[
\hat\epsilon_t^{\mathrm{guided}}(x_t) = \hat\epsilon_\theta(x_t,s,t) - \eta\sqrt{1-\bar\alpha_t}\,\nabla_{x_t}Q_t(s,x_t),
\qquad
...
\[
p_t^{\mathrm{tilt}}(x_t\mid s)\propto p_t(x_t\mid s)\,\exp\!\bigl(\eta\,Q_t(s,x_t)\bigr),
\]
...
\[
\hat\epsilon_t^{\mathrm{Tw}}(x_t) = \hat\epsilon_\theta(x_t,s,t) - \eta\sqrt{1-\bar\alpha_t}\,\nabla_{x_t}Q\bigl(s,\hat x_0(x_t,t)\bigr).
\]
...
\begin{itemize}
  ...
  \item as $t\to 0$, $\hat x_0(x_t,t)\to x_0$, so the clean tilted density $\pi'(a\mid s)\propto \pi_\theta(a\mid s)e^{\eta Q(s,a)}$ is recovered.
\end{itemize}
...
\medskip
If moves from $x$ to $y$ happen too often relative to the reverse moves, we restrict some of them using an acceptance probability $\alpha(x,y)$ so that detailed balance holds:
\[
\rho(x)\,q(y\mid x)\,\alpha(x,y)
=
\rho(y)\,q(x\mid y)\,\alpha(y,x).
\]
One choice that handles both directions at once is
\[
\alpha(x,y)=\min\!\left\{1,\frac{\rho(y)\,q(x\mid y)}{\rho(x)\,q(y\mid x)}\right\}.
\]
...
This is why MGMD uses an energy-based policy network. Now both ingredients are available:
 ...
 define the guided energy
 \[
 E_t^{\mathrm{guided}}(x_t)=E_\theta(s,x_t,t)-\eta\,Q\bigl(s,\hat x_0(x_t,t)\bigr).
 \]
 Therefore,
 \[
 \sqrt{1-\bar\alpha_t}\,\nabla_{x_t}E_t^{\mathrm{guided}}(x_t)
 = \hat\epsilon_\theta(s,x_t,t)-\eta\sqrt{1-\bar\alpha_t}\,\nabla_{x_t}Q\bigl(s,\hat x_0(x_t,t)\bigr)
 = \hat\epsilon_t^{\mathrm{Tw}}(x_t).
 \]
 \end{frame}
 ...
 \[
 \begin{aligned}
\alpha(x,x')&=\min\!\left\{1,\exp\!\left(E_t^{\mathrm{guided}}(x)-E_t^{\mathrm{guided}}(x')\right.
...
\STATE $E_t^{\mathrm{guided}}(x)\coloneqq E_\theta(s,x,t)-\eta\,\bar Q\bigl(s,\hat x_{0,r_Q}(x,t)\bigr)$
...
\FOR{$t=T,\dots,1$}
  ...
  \FOR{$m=1,\dots,M$}
    ...
    \STATE $\alpha\leftarrow\min\!\left\{1,\exp\!\bigl(E_t^{\mathrm{guided}}(x_t)-E_t^{\mathrm{guided}}(x')\bigr)\tfrac{\mathcal{N}(x_t;\mu',2s_tI)}{\mathcal{N}(x';\mu,2s_tI)}\right\}$ \COMMENT{MH accept prob.}
    \STATE Set $x_t\leftarrow x'$  with probability $\alpha$ \COMMENT{accept/reject}
