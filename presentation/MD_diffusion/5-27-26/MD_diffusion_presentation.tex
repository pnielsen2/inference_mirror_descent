...
\FOR{each environment step}
  ...
  \STATE $a \leftarrow \textsc{SampleFromTiltedPolicy}(s,\, \pi_\theta,\, \tfrac{1}{2}(Q_{\phi_1}{+}Q_{\phi_2}),\, \alpha,\,\beta)$ \COMMENT{Alg.~2}
  ...
  \STATE $a'_j \leftarrow \textsc{SampleFromTiltedPolicy}(s'_j,\, \pi_\theta,\, \tfrac{1}{2}(Q_{\phi_1}+Q_{\phi_2}),\, \alpha,\,\beta)$
  \STATE $y_j \leftarrow r_j + \gamma\,(1{-}d_j)\,\min_k(Q_{\bar{\phi}_k}(s'_j, a'_j))$
  \STATE Update each $Q_{\phi_k}$ by minimizing $\frac{1}{B}\sum_j (Q_{\phi_k}(s_j, a_j) - y_j)^2$
