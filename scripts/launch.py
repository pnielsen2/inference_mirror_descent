...
cd {project_dir}
# Ensure this codebase's local relax/ shadows any editable-install copy in
# site-packages (e.g. ~/.venvs/general's __editable__.relax-*.pth which is
# pinned to whichever codebase last ran `pip install -e .`).
export PYTHONPATH="{project_dir}:${{PYTHONPATH:-}}"

echo "Job ID: $SLURM_JOB_ID"
echo "Venv: $VIRTUAL_ENV"
echo "Project dir: {project_dir}"
echo "Loaded relax: $(python -c 'import relax, sys; sys.stdout.write(relax.__file__)')"
echo "Running: {cmd}"
...
def venv_for_command(cmd: str) -> Path:
...
def main():
    ...
    args_ablate = args.ablate
    args_oat_ablate = args.oat_ablate
    ...
    for i, pack in enumerate(packs):
        ...
        venv_path = venv_for_command(cmd)
