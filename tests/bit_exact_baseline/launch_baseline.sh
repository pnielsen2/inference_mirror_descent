...
# (5 kl_budgets x 8 seeds). Each run produces logs/<env>/dpmd_<ts>_s0_/episode_returns.csv
...
python scripts/launch.py \
  --cmd "python scripts/train_mujoco.py --alg dpmd --num_vec_envs 5 \
    --dpmd_constant_weight --tfg_eta 8.0 --num_particles 1 --mala_steps 2 \
    --q_critic_agg mean --beta_schedule_type cosine --beta_schedule_scale 1 \
    --dpmd_no_entropy_tuning --buffer_size 400000 --x0_hat_clip_radius 3.0 \
    --mala_adapt_rate 0.2 --mala_per_level_eta --q_td_huber_width 30.0 \
    --update_per_iteration 4 --lr_q 0.00015 --lr_policy 0.0003 \
    --mala_guided_predictor --ddim_predictor --kl_budget 1024 --tau 0.005 \
